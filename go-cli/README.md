...
boru-driven command-line client **and** interactive REPL for the SodeomAiProxy
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/sodeom-ai-proxy-cli)
make build

# 2. See usage (words, entities, env vars)
./sodeom-ai-proxy-cli --help

# 3. Provide credentials once, via the environment
export SODEOM_AI_PROXY_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./sodeom-ai-proxy-cli load 1 ain            # {id:1} shorthand
./sodeom-ai-proxy-cli load '{id:1}' ain       # explicit match map

# 5. Override the API base URL for a single call
SODEOM_AI_PROXY_BASE=https://api.example.com ./sodeom-ai-proxy-cli load 1 ain

# 6. No arguments -> interactive REPL
./sodeom-ai-proxy-cli
sodeom-ai-proxy> load 1 ain
sodeom-ai-proxy> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/sodeom-ai-proxy-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/sodeom-ai-proxy-cli load 1 ain
   ```
...
```sh
./sodeom-ai-proxy-cli load 1 ain          # scalar shorthand for {id:1}
./sodeom-ai-proxy-cli load '{id:1}' ain     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `sodeom-ai-proxy>`). Each line is
evaluated as its own boru expression:

```text
$ ./sodeom-ai-proxy-cli
sodeom-ai-proxy> load 1 ain
sodeom-ai-proxy> /help
sodeom-ai-proxy> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `ain`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/sodeom-ai-proxy-cli`. |
...
(`./sodeom-ai-proxy-cli <expr>`) and interactively (the REPL), and expressions compose the
