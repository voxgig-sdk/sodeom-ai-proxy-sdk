# SodeomAiProxy TypeScript SDK Reference

Complete API reference for the SodeomAiProxy TypeScript SDK.
...
```ts
new SodeomAiProxySDK(options?: object)
```
...
#### `SodeomAiProxySDK.test(testopts?, sdkopts?)`
...
**Returns:** `SodeomAiProxySDK` instance in test mode.
...
Create a new `Ain` entity instance.
...
Alias for `SodeomAiProxySDK.test()`.
...
```ts
const result = await client.Ain().load({ query: 'query' })
```
