...
//     (\`new SodeomAiProxySDK(...)\` / \`SodeomAiProxySDK.test(...)\` become
//     \`SodeomAiProxySDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'SodeomAiProxySDK'
...
const TEST_SEED = {"entity":{"ain":{"test01":{"id":"test01"}},"ain2":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new SodeomAiProxySDK(...)\` and any
// \`SodeomAiProxySDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+SodeomAiProxySDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/SodeomAiProxySDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
