...
describe('Ain2Entity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create']) {
      if (maybeSkipControl(t, 'entityOp', 'ain2.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set SODEOM_AI_PROXY_TEST_AIN__ENTID JSON to run live')
      return
    }
    ...
    // CREATE
    const ain2_ref01_ent = client.Ain2()
    let ain2_ref01_data = setup.data.new.ain2['ain2_ref01']

    ain2_ref01_data = await ain2_ref01_ent.create(ain2_ref01_data)
    assert(null != ain2_ref01_data)
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/ain2/Ain2TestData.json')
  ...
  const idmapEnvVal = process.env['SODEOM_AI_PROXY_TEST_AIN__ENTID']
