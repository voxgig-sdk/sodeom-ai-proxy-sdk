...
describe('AinDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new SodeomAiProxySDK({
  ...
  test('direct-load-ain', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-ain', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['SODEOM_AI_PROXY_TEST_AIN_ENTID']
