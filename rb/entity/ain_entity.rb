...
class AinEntity
  ...
  def make
    opts = @_entopts.dup
    AinEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = SodeomAiProxyHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Ain, Hash] the current Ain data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = SodeomAiProxyHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Ain fields)
  def match_get
  ...
  # Load a single Ain.
  #
  # @param reqmatch [AinLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Ain.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Ain, Hash] the loaded Ain; raises SodeomAiProxyError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = SodeomAiProxyHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  # Create a new Ain.
  #
  # @param reqdata [AinCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [Ain, Hash] the created Ain; raises SodeomAiProxyError on failure
