...
class AinEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set SODEOMAIPROXY_TEST_AIN_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    ain_ref01_data = nil
    if ain_ref01_data_raw.length > 0
    ...
    ain_ref01_match_dt0 = {}
    ain_ref01_data_dt0_loaded, err = ain_ref01_ent.load(ain_ref01_match_dt0, nil)
    assert_nil err
...
def ain_basic_setup(extra)
  ...
  entid_env_raw = ENV["SODEOMAIPROXY_TEST_AIN_ENTID"]
