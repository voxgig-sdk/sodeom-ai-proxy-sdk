# SodeomAiProxy Golang SDK



The Golang SDK for the SodeomAiProxy API — an entity-oriented client using standard Go conventions. No generics required; data flows as `map[string]any`.
...
## Install
```bash
go get github.com/voxgig-sdk/sodeom-ai-proxy-sdk/go@latest
```

The Go module proxy resolves the version from the `go/vX.Y.Z` GitHub
release tag — see [Releases](https://github.com/voxgig-sdk/sodeom-ai-proxy-sdk/releases) for the available versions.
...
```bash
go mod edit -replace github.com/voxgig-sdk/sodeom-ai-proxy-sdk/go=../sodeom-ai-proxy-sdk/go
```
...
func main() {
    ...
    ain, err := client.Ain(nil).Load(map[string]any{"id": "example_id"}, nil)
    if err != nil {
        panic(err)
    }
    fmt.Println(ain)
...
fmt.Println(ain) // the loaded mock data
```
...
```go
func TestSDK(testopts map[string]any, sdkopts map[string]any) *SodeomAiProxySDK
```

Creates a test-mode client with mock transport. Both arguments may be `nil`.

### SodeomAiProxySDK methods
...
All entities implement the `SodeomAiProxyEntity` interface.
...
Create an instance: `ain := client.Ain(nil)`
...
```go
ain, err := client.Ain(nil).Load(map[string]any{"id": "ain_id"}, nil)
if err != nil {
    panic(err)
}
...
```go
ain := client.Ain(nil)
ain.Load(map[string]any{"id": "example_id"}, nil)
