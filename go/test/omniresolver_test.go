...
import (
	...
	omni "github.com/voxgig-sdk/sodeom-ai-proxy-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.SodeomAiProxyError); is {
				out := map[string]any{
					"name":    "SodeomAiProxyError",
...
func omniCtx(arg any, client *sdk.SodeomAiProxySDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.SodeomAiProxySDK); is {
			client = live
