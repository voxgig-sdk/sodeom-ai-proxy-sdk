...
// SodeomAiProxy SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class SodeomAiProxyTestFeature extends SodeomAiProxyBaseFeature
{
    ...
    public function init(SodeomAiProxyContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (SodeomAiProxyContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = SodeomAiProxyParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (SodeomAiProxyContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(SodeomAiProxyContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = SodeomAiProxyParam::call($ctx, $k);
