...
// SodeomAiProxy SDK retry feature
...
class SodeomAiProxyRetryFeature extends SodeomAiProxyBaseFeature
{
    ...
    public function init(SodeomAiProxyContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (SodeomAiProxyContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(SodeomAiProxyContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(SodeomAiProxyContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
