...
//    only - a typed SodeomAiProxyContext there would make every ctx
...
final class SodeomAiProxyOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new SodeomAiProxyContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new SodeomAiProxySpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new SodeomAiProxyResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new SodeomAiProxyError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new SodeomAiProxyResponse($clean['response']);
    ...
    public static function err_from_map($m): ?SodeomAiProxyError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new SodeomAiProxyError(is_string($code) ? $code : '', $msg);
    }
