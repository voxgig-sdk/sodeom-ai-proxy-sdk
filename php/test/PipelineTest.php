...
// SodeomAiProxy SDK pipeline test
//
...
{
    private static function utility(): SodeomAiProxyUtility
    {
        return new SodeomAiProxyUtility();
    }
    ...
    private static function ctx(array $over = []): SodeomAiProxyContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new SodeomAiProxyContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new SodeomAiProxyOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof SodeomAiProxyError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new SodeomAiProxyOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = SodeomAiProxyMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new SodeomAiProxyOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = SodeomAiProxyMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new SodeomAiProxyOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = SodeomAiProxyMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new SodeomAiProxySpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = SodeomAiProxyMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new SodeomAiProxyResponse([]),
            'result' => new SodeomAiProxyResult([]),
        ]);
        [, $err] = SodeomAiProxyMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new SodeomAiProxySpec([]),
            'response' => null,
            'result' => new SodeomAiProxyResult([]),
        ]);
        [, $err] = SodeomAiProxyMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new SodeomAiProxySpec(['step' => 's']),
            'response' => new SodeomAiProxyResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new SodeomAiProxyResult([]),
        ]);
        [, $err] = SodeomAiProxyMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new SodeomAiProxySpec(['step' => 's']),
            'response' => new SodeomAiProxyResponse(self::resp(200, ['v' => 1])),
            'result' => new SodeomAiProxyResult([]),
        ]);
        [, $err] = SodeomAiProxyMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new SodeomAiProxySpec(['step' => 's']),
            'response' => new SodeomAiProxyResponse(self::resp(200, ['v' => 2])),
            'result' => new SodeomAiProxyResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        SodeomAiProxyMakeResponse::call($ctx);
    ...
    {
        $preset = new SodeomAiProxyResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new SodeomAiProxySpec([]),
            'response' => new SodeomAiProxyResponse([]),
            'result' => new SodeomAiProxyResult([]),
        ...
        [$got, $err] = SodeomAiProxyMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new SodeomAiProxyResult([])]);
        [, $err] = SodeomAiProxyMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new SodeomAiProxySpec([]), 'result' => null]);
        [, $err] = SodeomAiProxyMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new SodeomAiProxySpec(['step' => 's']),
            'result' => new SodeomAiProxyResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new SodeomAiProxyOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = SodeomAiProxyMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new SodeomAiProxySpec(['step' => 's']),
            'result' => new SodeomAiProxyResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new SodeomAiProxyOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = SodeomAiProxyMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new SodeomAiProxyResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new SodeomAiProxySpec([]),
            'result' => new SodeomAiProxyResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = SodeomAiProxyMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = SodeomAiProxyMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new SodeomAiProxyError('boom', 'boom');
        $utility->fetcher = function (SodeomAiProxyContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new SodeomAiProxySpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = SodeomAiProxyMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (SodeomAiProxyContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = SodeomAiProxyMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(SodeomAiProxyResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        SodeomAiProxyMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (SodeomAiProxyContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new SodeomAiProxySpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = SodeomAiProxyMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new SodeomAiProxyResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new SodeomAiProxySpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = SodeomAiProxyMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = SodeomAiProxyMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = SodeomAiProxyMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new SodeomAiProxyResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, SodeomAiProxyDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new SodeomAiProxyResult(['ok' => false])]);
        $this->expectException(SodeomAiProxyError::class);
        SodeomAiProxyDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new SodeomAiProxyResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', SodeomAiProxyMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        SodeomAiProxyMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new SodeomAiProxyBaseFeature();
        $b = new SodeomAiProxyBaseFeature();
        SodeomAiProxyFeatureAdd::call($ctx, $a);
        SodeomAiProxyFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): SodeomAiProxyBaseFeature
    {
        $f = new SodeomAiProxyBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        SodeomAiProxyFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): SodeomAiProxyContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new SodeomAiProxySpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = SodeomAiProxyPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        SodeomAiProxyPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new SodeomAiProxyResponse(['headers' => null]),
            'result' => new SodeomAiProxyResult([]),
        ]);
        SodeomAiProxyResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        SodeomAiProxyResultBody::call($ctx);
