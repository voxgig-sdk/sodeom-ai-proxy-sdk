...
// SodeomAiProxy SDK feature test
//
...
class FtCtrl extends SodeomAiProxyControl
{
...
class FtClient
{
    ...
    private SodeomAiProxyUtility $utility;

    public function __construct(SodeomAiProxyUtility $utility, array $options)
    {
    ...
    public function get_utility(): SodeomAiProxyUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (SodeomAiProxyContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public SodeomAiProxyContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (SodeomAiProxyContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = SodeomAiProxyConfig::make_feature($name);
    ...
    private static function buildUrl(SodeomAiProxySpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof SodeomAiProxyError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new SodeomAiProxyResponse($fetched);
        }
    ...
    private function fail(SodeomAiProxyContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (SodeomAiProxyContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
