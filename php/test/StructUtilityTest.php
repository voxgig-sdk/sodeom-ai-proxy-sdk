...
// SodeomAiProxy SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = SodeomAiProxyOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(SodeomAiProxyOmni::structfix($now, Struct::undef()))
                        !== json_encode(SodeomAiProxyOmni::structfix($val, Struct::undef()))) {
