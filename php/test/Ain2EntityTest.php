...
class Ain2EntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = SodeomAiProxySDK::test(null, null);
        $ent = $testsdk->Ain2(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = ain2_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "ain2." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set SODEOMAIPROXY_TEST_AIN__ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $ain2_ref01_ent = $client->Ain2(null);
        $ain2_ref01_data = Helpers::to_map(Vs::getprop(
            Vs::getpath($setup["data"], "new.ain2"), "ain2_ref01"));

        [$ain2_ref01_data_result, $err] = $ain2_ref01_ent->create($ain2_ref01_data, null);
        $this->assertNull($err);
        $ain2_ref01_data = Helpers::to_map($ain2_ref01_data_result);
        $this->assertNotNull($ain2_ref01_data);
...
function ain2_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/ain2/Ain2TestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["ain201", "ain202", "ain203"] as $k) {
    ...
    $entid_env_raw = getenv("SODEOMAIPROXY_TEST_AIN__ENTID");
