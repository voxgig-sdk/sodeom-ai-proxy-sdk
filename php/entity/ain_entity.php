...
class AinEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new AinEntity($this->_client, $opts);
    }

    /**
     * @param Ain|array $args Ain data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = SodeomAiProxyHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Ain|array The current Ain data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Ain fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = SodeomAiProxyHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Ain fields).
     */
     ...
     * Load a single Ain.
     *
     * @param AinLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed AinLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Ain|array The loaded Ain as an assoc-array at the
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = SodeomAiProxyHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     * Create a new Ain.
     *
     * @param AinCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed AinCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Ain|array The created Ain as an assoc-array at the
     *   SDK boundary; throws SodeomAiProxyError on failure (item-5 convention).
     */
