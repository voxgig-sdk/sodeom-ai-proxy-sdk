...
module SodeomAiProxyUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = SodeomAiProxyResponse.new({})
    ...
    if fetch_err
      ...
      response = SodeomAiProxyResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = SodeomAiProxyResponse.new(fetched)
    else
