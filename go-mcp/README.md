...
[MCP](https://modelcontextprotocol.io) server exposing the SodeomAiProxy SDK as
two agent tools — `sodeom-ai-proxy_list` and `sodeom-ai-proxy_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/sodeom-ai-proxy-mcp)
make build

# 2. Provide credentials via the environment
export SODEOM_AI_PROXY_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user sodeom-ai-proxy \
  -- /absolute/path/to/sodeom-ai-proxy-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./sodeom-ai-proxy-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// sodeom-ai-proxy_list: first page of records
{ "entity": "ain" }
{ "entity": "ain", "query": { } }

// sodeom-ai-proxy_load: one record by id
{ "entity": "ain", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/sodeom-ai-proxy-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user sodeom-ai-proxy \
     -- "$PWD"/dist/*/sodeom-ai-proxy-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `sodeom-ai-proxy_list` and `sodeom-ai-proxy_load` tools now appear
   in new sessions. Ask the agent to *"list ain using sodeom-ai-proxy"*
   and it calls `sodeom-ai-proxy_list` with `{"entity":"ain"}`.
...
```sh
export SODEOM_AI_PROXY_APIKEY=sk_live_xxx            # API key
export SODEOM_AI_PROXY_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `sodeom-ai-proxy_list` tool
...
### Call the `sodeom-ai-proxy_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `sodeom-ai-proxy_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `sodeom-ai-proxy_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `SODEOM_AI_PROXY_APIKEY` | API key sent with every request. |
| `SODEOM_AI_PROXY_BASE` | Optional override of the API base URL. |
...
```sh
./sodeom-ai-proxy-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"sodeom-ai-proxy_load","arguments":{"entity":"ain","query":{"id":1}}}}'
```
