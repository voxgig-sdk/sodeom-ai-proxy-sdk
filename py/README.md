# SodeomAiProxy Python SDK

The Python SDK for the SodeomAiProxy API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Ain(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.SodeomAiProxy(None).load(
    {"id": "test01"}, None
)
...
```
py/
├── sodeomaiproxy_sdk.py         -- Main SDK module
