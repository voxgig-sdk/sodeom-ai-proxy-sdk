...
from sodeomaiproxy_sdk.sodeomaiproxy_types import (
    Ain,
    AinLoadMatch,
    AinCreateData,
)
...
class AinEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return AinEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> Ain:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Ain().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: AinCreateData, ctrl=None) -> Ain:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
