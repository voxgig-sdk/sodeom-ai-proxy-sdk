...
class SodeomAiProxyTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            SodeomAiProxyTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = SodeomAiProxyTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = SodeomAiProxyTestRunner.getenv("SODEOM_AI_PROXY_TEST_LIVE")
        override = SodeomAiProxyTestRunner.getenv("SODEOM_AI_PROXY_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = SodeomAiProxyTestRunner.getenv(key)
        ...
        explain = SodeomAiProxyTestRunner.getenv("SODEOM_AI_PROXY_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if SodeomAiProxyTestRunner._test_control is not None:
            return SodeomAiProxyTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                SodeomAiProxyTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            SodeomAiProxyTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = SodeomAiProxyTestRunner.load_test_control()
...
def load_env_local():
    SodeomAiProxyTestRunner.load_env_local()


def env_override(m):
    return SodeomAiProxyTestRunner.env_override(m)


def entity_data(v):
    return SodeomAiProxyTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return SodeomAiProxyTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return SodeomAiProxyTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return SodeomAiProxyTestRunner.load_test_control()


def live_client_options():
    return SodeomAiProxyTestRunner.live_client_options()


def live_delay_ms():
    return SodeomAiProxyTestRunner.live_delay_ms()
