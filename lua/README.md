...
The Lua SDK for the SodeomAiProxy API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Ain(nil):load({ id = "example_id" }, nil)
...
local result, err = client:SodeomAiProxy(nil):load(
  { id = "test01" }, nil
)
