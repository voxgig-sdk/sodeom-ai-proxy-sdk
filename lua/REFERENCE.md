# SodeomAiProxy Lua SDK Reference

Complete API reference for the SodeomAiProxy Lua SDK.
...
| `messages` | `table` | Yes | Chat history array passed to the model |
...
```lua
local result, err = client:Ain():load({ query = "query" })
```
