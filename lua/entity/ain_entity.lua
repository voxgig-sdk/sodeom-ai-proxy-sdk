...
local AinEntity = {}
AinEntity.__index = AinEntity


function AinEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AinEntity)
  self._name = "ain"
  self._client = client
...
function AinEntity:get_name()
  return self._name
end


function AinEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AinEntity.new(self._client, opts)
end
...
function AinEntity:mark_deleted()
  self._deleted = true
end


function AinEntity:deleted()
  return true == self._deleted
end


function AinEntity:data_set(args)
  if args ~= nil then
...
function AinEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AinEntity:match_set(args)
  if args ~= nil then
...
function AinEntity:stream(action, args, callopts)
...
---@param reqmatch AinLoadMatch
---@param ctrl? table
---@return Ain
---@return string? err
function AinEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata AinCreateData
---@param ctrl? table
---@return Ain
---@return string? err
function AinEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function AinEntity:_run_op(ctx, post_done)
  local utility = self._utility
